...
npm run typecheck    # tsc (noEmit via tsconfig — only static check, no tests, no linter)
...
- **API Router** (`functions/api/[[path]].js`): Monolithic ~330-line router. Matches by `path.startsWith()` + string compare. Instantiates `UserService` + `GachaService` per request. Rate-limit on login (10/10min) and register (5/10min) via KV with `CF-Connecting-IP`.
...
Direct SQL, no ORM. `STRICT` mode, foreign keys cascade on delete. Integer millisecond timestamps (`Date.now()`). 11 tables: `users`, `gallery`, `inventory`, `logs`, `level_rewards`, `user_titles`, `user_uploads`, `draw_history`, `card_likes`, `card_bookmarks`, `buffer_claims`.
